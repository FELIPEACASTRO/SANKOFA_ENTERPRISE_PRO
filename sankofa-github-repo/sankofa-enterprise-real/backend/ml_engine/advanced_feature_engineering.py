...
class AdvancedFeatureEngineering:
    ...
    def _create_client_behavior_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'client_cpf' in df.columns and 'value' in df.columns:
            ...
            client_stats.columns = ['client_cpf', 'avg_value', 'std_value', 'num_transactions', 'max_value']
    ...
    def _create_device_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'device_id' in df.columns:
            ...
            df['is_new_device'] = df['device_id'].str.contains('new', case=False, na=False).astype(int)
    ...
    def get_feature_names(self) -> List[str]:
        ...
        return [
            # Temporais
            'hour', 'day_of_week', 'is_weekend', 'is_night', 'is_business_hours', 'is_early_morning',
            # Valor
            'log_value', 'value_rounded', 'is_high_value', 'is_very_high_value',
            # Comportamento
            'avg_value', 'std_value', 'num_transactions', 'value_deviation', 'is_new_client', 'is_max_value',
            # Dispositivo
            'num_clients_per_device', 'is_shared_device', 'is_new_device',
            # Localização
            'is_high_risk_state', 'is_brazil',
            # Canal/Tipo
            'is_mobile', 'is_web', 'is_atm', 'is_pix', 'is_boleto', 'is_credit',
            # Velocidade
            'time_since_last_transaction', 'is_rapid_transaction', 'is_very_rapid_transaction'
        ]
