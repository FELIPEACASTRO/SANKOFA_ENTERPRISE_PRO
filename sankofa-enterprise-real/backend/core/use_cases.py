...
class ProcessTransactionUseCase:
    ...
    async def _apply_business_rules(
        ...
        if (
            transaction.is_high_value(Money(Decimal("5000")))
            and customer.get_transaction_count() < 5
        ):
            return "manual_review_required"
    ...
    async def _handle_notifications(
        self, transaction: Transaction, fraud_analysis: FraudAnalysisResult, decision: str
    ) -> None:
    ...
    def _record_metrics(
        self, decision: str, fraud_analysis: FraudAnalysisResult, duration: float
    ) -> None:
        """Record metrics - O(1)"""
    ...
    async def execute(self, command: ApproveTransactionCommand) -> Dict[str, Any]:
        ...
        try:
            ...
            self._metrics_collector.record_histogram(
                "approval_processing_duration", (time.time() - start_time) * 1000
            )
...
class GetTransactionUseCase:
    ...
    def __init__(self, transaction_repo: TransactionRepository, cache_service: CacheService):
        self._transaction_repo = transaction_repo
        self._cache_service = cache_service
...
class GetFraudStatisticsUseCase:
    ...
    async def execute(self, query: GetFraudStatisticsQuery) -> Dict[str, Any]:
        ...
        transactions = await self._transaction_repo.find_by_date_range(
            query.start_date, query.end_date, limit=10000  # Reasonable limit
        )
        ...
        fraud_amount = sum(
            float(t.amount.amount) for t in transactions if t.risk_level == RiskLevel.CRITICAL
        )

        # Calculate rates - O(1)
        fraud_rate = (
            (fraud_transactions / total_transactions * 100) if total_transactions > 0 else 0
        )
...
class TransactionProcessingSaga:
    ...
    def __init__(self, process_use_case: ProcessTransactionUseCase, audit_service: AuditService):
        self._process_use_case = process_use_case
        self._audit_service = audit_service
        self._steps = []
        self._compensations = []
