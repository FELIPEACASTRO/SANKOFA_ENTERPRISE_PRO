...
class TestProcessTransactionUseCase:
    ...
    def sample_customer(self):
        ...
        return Customer(id="CUSTOMER_456", email="test@example.com", created_at=datetime.utcnow())
    ...
    async def test_process_low_risk_transaction(
        self, use_case, mock_dependencies, sample_command, sample_customer, low_risk_fraud_result
    ):
    ...
    async def test_process_high_risk_transaction(
        self, use_case, mock_dependencies, sample_command, sample_customer, high_risk_fraud_result
    ):
    ...
    async def test_process_high_value_new_customer(
        self, use_case, mock_dependencies, sample_customer, low_risk_fraud_result
    ):
    ...
    async def test_new_customer_creation(
        self, use_case, mock_dependencies, sample_command, low_risk_fraud_result
    ):
    ...
    @pytest.mark.asyncio
    async def test_error_handling(self, use_case, mock_dependencies, sample_command):
        """Test error handling in use case"""
        # Setup fraud service to raise exception
        mock_dependencies["fraud_service"].analyze_transaction.side_effect = Exception(
            "ML service error"
        )
        mock_dependencies["cache_service"].get.return_value = None
        mock_dependencies["customer_repo"].find_by_id.return_value = None
        ...
        mock_dependencies["metrics_collector"].increment_counter.assert_called_with(
...
class TestApproveTransactionUseCase:
    ...
    @pytest.mark.asyncio
    async def test_approve_valid_transaction(self, use_case, mock_dependencies, sample_transaction):
        """Test approving valid transaction"""
        command = ApproveTransactionCommand(
            transaction_id="TXN_123456789", approved_by="admin_user"
        )
    ...
    @pytest.mark.asyncio
    async def test_approve_nonexistent_transaction(self, use_case, mock_dependencies):
        """Test approving non-existent transaction"""
        command = ApproveTransactionCommand(
            transaction_id="TXN_NONEXISTENT", approved_by="admin_user"
        )
    ...
    async def test_approve_critical_risk_transaction(
        self, use_case, mock_dependencies, sample_transaction
    ):
...
class TestGetTransactionUseCase:
    ...
    def mock_dependencies(self):
        """Create mock dependencies"""
        return {"transaction_repo": AsyncMock(), "cache_service": AsyncMock()}
    ...
    @pytest.mark.asyncio
    async def test_get_transaction_cache_hit(self, use_case, mock_dependencies):
    ...
    @pytest.mark.asyncio
    async def test_get_nonexistent_transaction(self, use_case, mock_dependencies):
...
class TestGetFraudStatisticsUseCase:
    ...
    async def test_get_fraud_statistics_cache_miss(
        self, use_case, mock_dependencies, sample_transactions
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_fraud_statistics_cache_hit(self, use_case, mock_dependencies):
        """Test getting fraud statistics with cache hit"""
        start_date = datetime(2023, 1, 1)
        end_date = datetime(2023, 1, 31)
        query = GetFraudStatisticsQuery(start_date, end_date)

        cached_result = {"summary": {"fraud_rate_percent": 15.0}, "cached": True}
    ...
    @pytest.mark.asyncio
    async def test_get_fraud_statistics_empty_results(self, use_case, mock_dependencies):
