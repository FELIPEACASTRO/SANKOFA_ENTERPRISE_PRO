    ...
    def get_sequence_features(self, user_id: str) -> Optional[np.ndarray]:
        ...
        for txn in sequence:
            features.append([
                txn.get('amount', 0),
                txn.get('hour', 12),
                1 if txn.get('is_fraud', False) else 0
            ])
...
class BiLSTMSequenceAnalyzer:
    ...
    def _build_model(self, n_features: int) -> None:
        ...
        logger.info(f"Bi-LSTM model built: {self.sequence_length}x{n_features} -> {self.lstm_units*2}")
    ...
    def update_user_baseline(self, user_id: str, transaction: Dict[str, Any]) -> None:
        ...
        if n > 0:
            new_std = np.sqrt(
                (baseline['amount_std']**2 * n + (amount - new_avg)**2) / (n + 1)
            )
        else:
            new_std = 0
