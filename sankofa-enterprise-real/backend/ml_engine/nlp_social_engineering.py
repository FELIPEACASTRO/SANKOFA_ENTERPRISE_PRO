...
class NLPSocialEngineeringDetector:
    ...
    def _compile_patterns(self):
        ...
        self.compiled_emotional = [re.compile(p, re.IGNORECASE) for p in self.EMOTIONAL_PATTERNS]
        self.compiled_bank = [
        ...
        self.compiled_pix = [re.compile(p, re.IGNORECASE) for p in self.PIX_FRAUD_PATTERNS]
    ...
    def analyze_text(
        ...
        high_risk_count = sum(
            1
            for s in [urgency_score, bank_score, phishing_score, pix_score, whatsapp_score]
            if s > 0.3
        )
    ...
    def batch_analyze(
        self, texts: List[str], source: Optional[str] = None
    ) -> List[SocialEngineeringAnalysis]:
        """Analisa múltiplos textos"""
        results = []
        for i, text in enumerate(texts):
            result = self.analyze_text(text=text, text_id=f"BATCH_{i:04d}", source=source)
