...
class DatasetLoader:
    ...
    def load_credit_card_ulb(self, encoding: str = 'utf-8') -> pd.DataFrame:
        ...
        try:
            ...
            df = data['frame'] if isinstance(data, dict) else pd.DataFrame(data.data, columns=data.feature_names)
    ...
    def _add_pix_features(self, df: pd.DataFrame, synthetic: bool = True) -> pd.DataFrame:
        ...
        df['unregistered_device_exceeded'] = (
        ...
        logger.warning("Features PIX SINTÉTICAS adicionadas ao dataset (use dados reais em produção)")
        return df
