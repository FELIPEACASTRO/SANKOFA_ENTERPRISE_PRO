...
class DeviceFingerprintAnalyzer:
    ...
    EMULATOR_KEYWORDS = [
        ...
        'vmware', 'bluestacks', 'genymotion', 'nox',
        'android emulator', 'google android sdk'
    ]
    ...
    def _check_multiple_users(self, fingerprint: str, user_id: str) -> Dict:
        ...
        if len(other_users) >= 3:
            result['detected'] = True
            result['score'] = 0.40
            result['reasons'].append(f"Dispositivo usado por {len(other_users) + 1} usuários diferentes")
    ...
    def _hash_components(self, components: Dict) -> Dict[str, str]:
        ...
        for key, value in components.items():
            data = json.dumps(value, sort_keys=True) if isinstance(value, (dict, list)) else str(value)
