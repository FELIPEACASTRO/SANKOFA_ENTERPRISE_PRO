...
class EnhancedFraudEngineV4:
    ...
    def _create_preprocessor(self, X_sample):
        ...
        if "isFlaggedFraud" in numeric_features:
            numeric_features.remove("isFlaggedFraud")
    ...
    def _stratified_sample_df(self, df, sample_size_per_class, random_state):
        ...
        n_non_fraud_samples = min(
            len(df_non_fraud), sample_size_per_class * 10
        )  # Example: 10x non-fraud for each fraud sample
        ...
        logging.info(
            f"  Amostragem estratificada: {len(df_fraud_sampled)} fraudes, {len(df_non_fraud_sampled)} não-fraudes."
        )
        return sampled_df
    ...
    def train_model(self, datasets_dict):
        ...
        for name, df in datasets_dict.items():
            ...
            sample_for_preprocessor.append(
                self._stratified_sample_df(
                    df, sample_size_per_class=100, random_state=self.random_state
                )
            )
        ...
        self._create_preprocessor(
            combined_sample_for_preprocessor.drop(columns=["isFraud"], errors="ignore")
        )
        ...
        for name, df in datasets_dict.items():
            ...
            sampled_df = self._stratified_sample_df(
                df_copy, self.sample_size_per_class, self.random_state
            )
        ...
        logging.info(
            f"Contagem de classes de y_train_res após balanceamento:\n{pd.Series(y_train_res).value_counts()}"
        )
        ...
        if len(high_precision_thresholds) > 0:
            ...
            logging.info(
                f"Limiar de decisão ótimo para precisão >= 0.999: {self.optimal_threshold:.4f}"
            )
        else:
            logging.warning(
                "Não foi possível encontrar um limiar com precisão >= 0.999. Usando limiar padrão de 0.5."
            )
    ...
    def predict_proba(self, X_new):
        ...
        expected_features = (
            self.model_pipeline["numerical_features"] + self.model_pipeline["categorical_features"]
        )
        for col in expected_features:
...
if __name__ == "__main__":
    ...
    datasets = {"creditcard_kaggle": df_kaggle_processed, "brasileiro_sintetico": df_br_processed}
