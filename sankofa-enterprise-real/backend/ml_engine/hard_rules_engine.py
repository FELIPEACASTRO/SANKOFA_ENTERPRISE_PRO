...
class HardRulesEngine:
    ...
    OPERATORS = {
        ...
        "between": lambda a, b: (
            float(b.split(",")[0]) <= float(a) <= float(b.split(",")[1]) if "," in str(b) else False
        ),
    ...
    def evaluate(self, transaction: Dict) -> HardRulePrediction:
        ...
        transaction_id = transaction.get(
            "id", transaction.get("transaction_id", f"TXN_{int(time.time()*1000)}")
        )
...
class UnifiedFraudEngine:
    ...
    def evaluate(
        self, transaction: Dict, use_ml: bool = True, use_rules: bool = True
    ) -> Dict[str, Any]:
        """
