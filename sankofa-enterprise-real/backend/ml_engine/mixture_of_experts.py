...
class MixtureOfExperts:
    ...
    EXPERT_FEATURES = {
        ExpertType.TRANSACTION_PATTERN: [
            'amount', 'transaction_amount', 'avg_amount_7d', 'avg_amount_30d',
            'amount_deviation', 'transaction_count_7d'
        ],
        ExpertType.BEHAVIORAL_ANOMALY: [
            'amount_deviation', 'velocity_deviation', 'time_since_last_txn',
        ...
        ExpertType.DEVICE_FINGERPRINT: [
            ...
            'device_transaction_count', 'is_rooted_device'
        ],
        ExpertType.SOCIAL_ENGINEERING: [
            'urgency_score', 'phishing_probability', 'social_engineering_risk',
            'message_risk_score', 'call_before_transaction'
        ],
        ExpertType.PIX_SPECIFIC: [
            'pix_key_type_encoded', 'receiver_account_age', 'is_first_pix_to_receiver',
            'pix_hour_risk', 'dict_check_result'
        ],
        ExpertType.HIGH_VALUE: [
            'amount', 'transaction_amount', 'amount_percentile',
            'is_above_limit', 'amount_vs_avg_ratio'
        ],
        ExpertType.NIGHT_TRANSACTION: [
            'hour', 'transaction_hour', 'is_night', 'is_weekend',
            'hour_risk_score', 'typical_hour_deviation'
        ]
    }
    ...
    def __init__(
        ...
        logger.info(f"MixtureOfExperts initialized (v{self.VERSION}) with {len(self.experts)} experts")
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series) -> Dict[str, Any]:
        ...
        if self.use_gating:
            ...
            expert_performance = np.random.randint(
                0, len(self.experts), size=len(X)
            )
        ...
        return {
            "experts_trained": len([r for r in training_results.values() if r.get("status") == "trained"]),
    ...
    def predict(self, transaction: Dict[str, Any]) -> MoEResult:
        ...
        final_probability = self._aggregate_predictions(
    ...
    def _aggregate_predictions(
        ...
        if self.aggregation_method == "weighted_average":
            ...
            valid_probs = [
                p.fraud_probability for p in predictions
