...
class CatBoostFraudModel:
    ...
    def preprocess_data(
        ...
        cat_indices = [
            all_features.index(f) for f in self.categorical_features if f in all_features
        ]
    ...
    def train(
        ...
        train_pool = Pool(data=X_processed, label=y_train, cat_features=cat_indices)

        eval_pool = None
        if X_val is not None and y_val is not None:
            X_val_processed, _ = self.preprocess_data(X_val, fit=False)
            eval_pool = Pool(data=X_val_processed, label=y_val, cat_features=cat_indices)

        self.model.fit(
            train_pool, eval_set=eval_pool, use_best_model=eval_pool is not None, plot=False
        )
        ...
        logger.info(
            f"CatBoost trained - AUC: {metrics['roc_auc']:.4f}, F1: {metrics['f1_score']:.4f}"
        )
...
class CatBoostEnsembleIntegration:
    ...
    def get_ensemble_prediction(
        self, X: pd.DataFrame, other_predictions: Dict[str, np.ndarray]
    ) -> np.ndarray:
        """
