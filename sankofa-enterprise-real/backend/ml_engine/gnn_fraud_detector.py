...
class TransactionGraph:
    ...
    def detect_suspicious_patterns(self, customer_id: str) -> List[str]:
        ...
        if neighbor_fraud_rate > 0.2:
            patterns.append(f"FRAUD_NETWORK: {neighbor_fraud_rate:.1%} da rede com histórico de fraude")
...
class GNNFraudDetector:
    ...
    def add_historical_transactions(self, transactions: pd.DataFrame):
        ...
        for _, row in transactions.iterrows():
            self.transaction_graph.add_transaction(
                ...
                receiver_id=str(row.get('receiver_id', '')) if pd.notna(row.get('receiver_id')) else None,
                ...
                merchant_id=str(row.get('merchant_id', '')) if pd.notna(row.get('merchant_id')) else None,
    ...
    def analyze_transaction(
        ...
        **kwargs
        ...
        graph_risk_score = sum(
            risk_factors[key] * self.pattern_weights[key]
    ...
    def get_graph_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'communities': len(set(self.transaction_graph.community_cache.values())) if self.transaction_graph.community_cache else 0,
