...
class TransferLearningPipeline:
    ...
    def load_dataset(
        self, dataset_key: str, sample_size: Optional[int] = None, local_path: Optional[str] = None
    ...
    def transfer_learn(
        ...
        source_df = self.load_dataset(
            source_dataset, sample_size=source_sample_size, local_path=source_local_path
        )
        ...
        improvement = {
            k: target_metrics.get(k, 0) - source_metrics.get(k, 0) for k in source_metrics.keys()
        }

        model_name = (
            f"transfer_{source_dataset}_to_target_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
    ...
    def get_dataset_compatibility(self, dataset_key: str) -> Dict[str, Any]:
    ...
    def list_supported_datasets(self) -> Dict[str, Dict]:
        ...
        return {key: self.get_dataset_compatibility(key) for key in self.SUPPORTED_DATASETS.keys()}
