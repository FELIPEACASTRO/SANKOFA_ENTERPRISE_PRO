...
class AdvancedModulesOrchestrator:
    ...
    def _determine_enrichment_level(
        self, base_probability: float, force_full: bool
    ) -> EnrichmentLevel:
    ...
    def _enrich_parallel(
        ...
        if level in [EnrichmentLevel.STANDARD, EnrichmentLevel.FULL]:
            if self._autoencoder:
                futures["autoencoder"] = self.executor.submit(self._run_autoencoder, transaction)

            if self._moe:
                futures["moe"] = self.executor.submit(self._run_moe, transaction)
        ...
        if level == EnrichmentLevel.FULL:
            ...
            if self._gnn:
                futures["gnn"] = self.executor.submit(self._run_gnn, transaction)
    ...
    def _run_sequence_analysis(self, user_id: str, transaction: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _run_gnn(self, transaction: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "graph_risk": (
                    result.graph_risk_score if hasattr(result, "graph_risk_score") else 0.0
                ),
                "community_risk": (
                    result.community_risk if hasattr(result, "community_risk") else 0.0
                ),
                "suspicious_patterns": (
                    result.suspicious_patterns if hasattr(result, "suspicious_patterns") else []
                ),
            }
        except Exception as e:
    ...
    def _run_explainer(
        self, transaction: Dict[str, Any], prediction: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _apply_result(
        self, enriched: EnrichedPrediction, module_name: str, result: Dict[str, Any]
    ) -> None:
