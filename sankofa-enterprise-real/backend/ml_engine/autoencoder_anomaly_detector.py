...
class AutoencoderAnomalyDetector:
    ...
    def _build_model(self, input_dim: int) -> None:
        ...
        self.model.compile(
            optimizer=keras.optimizers.Adam(learning_rate=self.learning_rate), loss="mse"
        )

        logger.info(
            f"Autoencoder built: {input_dim} -> {self.hidden_layers} -> {self.encoding_dim}"
        )
    ...
    def detect_anomaly(self, transaction: Dict[str, Any]) -> AnomalyResult:
        ...
        top_contributors = sorted(feature_contributions.items(), key=lambda x: x[1], reverse=True)[
            :5
        ]
        ...
        if is_anomaly:
            if percentile_rank > 99:
                ...
                "velocity" in f.lower() or "frequency" in f.lower() for f, _ in top_contributors[:2]
            ):
