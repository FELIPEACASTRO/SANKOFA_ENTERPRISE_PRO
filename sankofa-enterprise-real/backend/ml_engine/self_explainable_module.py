...
class InterpretativeMaskLearner:
    ...
    LGPD_SENSITIVE_FIELDS = {
        'cpf', 'customer_id', 'device_id', 'ip_address', 'phone',
        'email', 'name', 'address', 'location', 'pix_key'
    }
    ...
    def generate_explanation(
        ...
        explanation_text = self._generate_natural_language(
            transaction, is_fraud, fraud_probability, 
            feature_importance, rule_triggers, behavioral_deviations
        )
        
        audit_trail = self._create_audit_trail(
            transaction_id, transaction, is_fraud, 
            fraud_probability, feature_importance
        )
    ...
    def _generate_natural_language(
        ...
        if top_features:
            ...
            explanation_parts.append(
                f"Principais fatores: {', '.join(feature_explanations)}."
            )
        
        if rule_triggers:
            explanation_parts.append(
                f"Regras ativadas: {'; '.join(rule_triggers[:3])}."
            )
    ...
    def _create_audit_trail(
        ...
        audit_entry = {
            ...
            "feature_importance_summary": dict(sorted(
                feature_importance.items(),
                key=lambda x: x[1],
                reverse=True
            )[:5]),
