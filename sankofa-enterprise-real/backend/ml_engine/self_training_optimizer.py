...
class SelfTrainingClassifier(BaseEstimator, ClassifierMixin):
    ...
    def fit(
        self, X_labeled: np.ndarray, y_labeled: np.ndarray, X_unlabeled: np.ndarray = None
...
class AdaptiveSelfTraining:
    ...
    def fit(
        ...
        for iteration in range(max_iter):
            ...
            high_confidence_mask = (probs >= self.current_threshold) | (
                probs <= (1 - self.current_threshold)
            )
...
def create_unlabeled_from_production(
    df: pd.DataFrame, labeled_mask: np.ndarray = None, sample_ratio: float = 0.3
...
if __name__ == "__main__":
    ...
    clf = SelfTrainingClassifier(
        threshold=0.95, max_iter=5, fraud_threshold=0.9, non_fraud_threshold=0.1
    )
