...
class PIXFraudTaxonomy:
    ...
    def analyze_transaction(
        ...
        **kwargs,
        ...
        receiver_indicators = self._analyze_receiver_patterns(receiver_id, historical_data)
        ...
        for fraud_type, rules in self.FRAUD_RULES.items():
            score = self._calculate_fraud_type_score(fraud_type, rules, detected_indicators)
        ...
        high_weight_indicators = [
            ind for ind in detected_indicators if ind.weight >= 0.5 and ind.detected
        ]
        ...
        compliance_flags = self._generate_compliance_flags(amount, timestamp, predicted_type)
    ...
    def _analyze_temporal_patterns(
        self, timestamp: datetime, amount: float
    ...
    def _analyze_amount_patterns(
        self, amount: float, historical_data: Dict
    ) -> List[PIXFraudIndicator]:
    ...
    def _analyze_receiver_patterns(
        self, receiver_id: str, historical_data: Dict
    ) -> List[PIXFraudIndicator]:
    ...
    def _analyze_device_patterns(self, device_info: Dict) -> List[PIXFraudIndicator]:
    ...
    def _calculate_fraud_type_score(
        self, fraud_type: PIXFraudType, rules: Dict, detected_indicators: List[PIXFraudIndicator]
    ) -> float:
        ...
        matched_indicators = [ind for ind in detected_indicators if ind.indicator_type in matched]
        avg_confidence = (
            np.mean([ind.confidence for ind in matched_indicators]) if matched_indicators else 0.5
        )
    ...
    def _generate_explanation(
        self, fraud_type: PIXFraudType, indicators: List[PIXFraudIndicator], probability: float
    ) -> str:
...
Sankofa Enterprise Pro, em conformidade com as regulamentações BACEN e LGPD.
    ...
    def _generate_compliance_flags(
        self, amount: float, timestamp: datetime, fraud_type: PIXFraudType
    ) -> List[str]:
