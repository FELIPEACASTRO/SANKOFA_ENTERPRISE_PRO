...
class UltraPrecisionFraudEngineV4:
    ...
    def __init__(self, random_state=42):
        ...
        self.base_models = {
            ...
            "lr": LogisticRegression(
                random_state=self.random_state, solver="liblinear", class_weight="balanced", C=0.1
            ),
            "svm": SVC(
                random_state=self.random_state,
                probability=True,
        ...
        self.meta_model = LogisticRegression(
            random_state=self.random_state, solver="liblinear", C=1.0
        )
    ...
    def _advanced_feature_engineering(self, df):
        ...
        df = pd.get_dummies(
            df, columns=[col for col in categorical_cols if col in df.columns], drop_first=True
        )
    ...
    def _advanced_preprocessing(self, df, fit=True):
        ...
        temporal_features = [
            ...
            if any(
                x in col.lower() for x in ["hour", "day", "month", "weekend", "night", "business"]
            )
        ]
        geo_features = [
            col
            for col in df.columns
            if any(x in col.lower() for x in ["latitude", "longitude", "distance"])
        ]
        categorical_features = [
            col
            for col in df.columns
            if col not in value_features + temporal_features + geo_features
        ]
    ...
    def train(self, X_train, y_train):
        ...
        logging.info(
            f"Features selecionadas: {len(self.selected_features)} de {len(self.feature_names)}"
        )
        ...
        logging.info(
            f"Dados após SMOTE: {len(X_resampled)} amostras, {np.sum(y_resampled)} fraudes."
        )
        ...
        for name, model in self.base_models.items():
            ...
            logging.info(
                f"CV F1-Score para {name}: {cv_scores.mean():.4f} (+/- {cv_scores.std() * 2:.4f})"
            )
    ...
    def _advanced_threshold_calibration(self, X_val, y_val):
        ...
        logging.info(
            f"Threshold Precision otimizado: {self.precision_threshold:.4f} (Precision: {best_precision:.4f})"
        )
    ...
    def predict_proba(self, X_input):
        ...
        if not self.is_trained:
            raise ValueError(
                "O motor não foi treinado. Por favor, treine o motor antes de fazer predições."
            )
        ...
        X_processed = self._advanced_feature_engineering(
            df_test.drop(columns=["is_fraud", "fraud_score"], errors="ignore")
        )
    ...
    def analyze_transaction(
        self, transaction: Transaction, use_precision_threshold=False
    ) -> Dict[str, Any]:
    ...
    def generate_realistic_fraud_data(num_samples=50000, fraud_ratio=0.02):
        ...
        data = {
            ...
            "tipo_transacao": np.random.choice(
                ["PIX", "DEBITO", "CREDITO", "TED", "DOC"],
                num_samples,
                p=[0.45, 0.25, 0.15, 0.1, 0.05],
            ),
            "canal": np.random.choice(
                ["MOBILE", "WEB", "POS", "ATM"], num_samples, p=[0.6, 0.25, 0.1, 0.05]
            ),
            ...
            "timestamp": pd.date_range(start="2024-01-01", periods=num_samples, freq="30S")
            .astype(str)
            .tolist(),
            "latitude": np.random.normal(
                -15, 8, num_samples
            ),  # Distribuição mais realista para Brasil
        ...
        if not fraud_indices.empty:
            ...
            df.loc[night_fraud, "timestamp"] = (
                pd.date_range(start="2024-01-01 02:00:00", periods=len(night_fraud), freq="1H")
                .astype(str)
                .tolist()
            )
            ...
            df.loc[geo_fraud, "latitude"] = np.random.uniform(
                40, 60, len(geo_fraud)
            )  # Fora do Brasil
