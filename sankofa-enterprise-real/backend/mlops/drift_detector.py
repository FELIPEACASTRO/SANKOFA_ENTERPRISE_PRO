...
class DriftDetector:
    ...
    def detect_concept_drift(
        ...
        if "prediction_distribution" in reference_stats:
            ...
            drift_score = self._calculate_distribution_drift(reference_pred_dist, current_pred_dist)
    ...
    def _calculate_data_drift_score(
    ...
    def _calculate_categorical_drift_score(
        ...
        try:
            ...
            # Contar frequências
            ref_counts = (
                pd.Series(reference_data).value_counts().reindex(all_categories, fill_value=0)
            )
            cur_counts = (
                pd.Series(current_data).value_counts().reindex(all_categories, fill_value=0)
            )
    ...
    def get_drift_summary(self, hours_back: int = 24) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "data_drifts": len(
                [d for d in recent_drifts if d.drift_type == "data_drift" and d.is_drift_detected]
            ),
            "concept_drifts": len(
                [
                    d
                    for d in recent_drifts
                    if d.drift_type == "concept_drift" and d.is_drift_detected
                ]
            ),
            "severity_breakdown": {
    ...
    def _get_most_affected_features(self, drifts: List[DriftReport]) -> List[Dict[str, Any]]:
        ...
        for drift in drifts:
            if drift.is_drift_detected:
                ...
                feature_drifts[drift.feature_name]["max_score"] = max(
                    feature_drifts[drift.feature_name]["max_score"], drift.drift_score
                )
        ...
        sorted_features = sorted(
            feature_drifts.items(), key=lambda x: x[1]["avg_score"], reverse=True
        )
