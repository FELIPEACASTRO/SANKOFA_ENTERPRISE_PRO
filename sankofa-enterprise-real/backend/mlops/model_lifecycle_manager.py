...
class ModelLifecycleManager:
    ...
    def register_model_version(
        ...
        logger.info(
            f"🎯 Métricas: Acc={metrics.accuracy:.3f}, Prec={metrics.precision:.3f}, "
    ...
    def promote_to_production(self, model_name: str, version: str) -> bool:
        ...
        logger.info(
            f"✅ Métricas de produção: Acc={metrics.accuracy:.3f}, "
    ...
    def record_performance_metrics(self, metrics: ModelMetrics):
        ...
        self.performance_history = [
            m for m in self.performance_history if datetime.fromisoformat(m.timestamp) > cutoff_time
        ]
        ...
        logger.debug(
            f"📊 Métricas registradas: Acc={metrics.accuracy:.3f}, "
    ...
    def _check_retraining_triggers(self, current_metrics: ModelMetrics):
        ...
        for trigger in triggers:
            logger.warning(
                f"⚠️ Gatilho de retreinamento: {trigger.trigger_type} "
    ...
    def get_model_performance_summary(self, model_name: str = None) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "active_triggers": len(
                [
                    t
                    for t in self.retraining_triggers
                    if datetime.fromisoformat(t.triggered_at) > datetime.now() - timedelta(hours=1)
                ]
            ),
    ...
    def _check_model_health(self, model_name: str, model_version: ModelVersion):
        ...
        if not os.path.exists(model_version.model_path):
            logger.error(
                f"❌ Arquivo do modelo {model_name} {model_version.version} não encontrado"
            )
            return
        ...
        if age_hours > 24 * 7:  # Mais de 7 dias
            logger.warning(
                f"⏰ Modelo {model_name} {model_version.version} está antigo ({age_hours:.1f}h)"
            )
    ...
    def _save_model_registry(self):
        ...
        registry_data = {
            ...
            "current_production_models": {
                name: asdict(model) for name, model in self.current_production_models.items()
            },
            "last_updated": datetime.now().isoformat(),
        }
    ...
    def _save_performance_metrics(self, metrics: ModelMetrics):
        ...
        metrics_file = os.path.join(
            self.metrics_dir, f"performance_{datetime.now().strftime('%Y%m%d')}.jsonl"
        )
...
if __name__ == "__main__":
    ...
    version = manager.register_model_version(
        "fraud_detector", "models/fraud_detector_v1.pkl", "abc123", test_metrics
    )
