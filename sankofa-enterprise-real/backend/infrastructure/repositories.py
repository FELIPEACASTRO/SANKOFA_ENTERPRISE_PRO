...
from ..core.entities import (
    Transaction, TransactionId, Customer, Money, 
    TransactionStatus, RiskLevel, DomainEvent
)
...
class RedisTransactionRepository(TransactionRepository):
    ...
    async def save(self, transaction: Transaction) -> None:
        ...
        await self._redis.zadd(
            customer_key, 
            {transaction.id.value: transaction.timestamp.timestamp()}
        )
...
class CompositeTransactionRepository(TransactionRepository):
    ...
    def __init__(
        self, 
        primary_repo: PostgreSQLTransactionRepository,
