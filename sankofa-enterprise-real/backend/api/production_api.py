        ...
        try:
            ...
            if not database_url:
                if self._fail_closed:
                    raise DatabaseError(
                        "DATABASE_URL required in production mode", context={"fail_closed": True}
                    )
            ...
            logger.info(
                "PostgreSQL connection pool initialized", pool_min=pool_min, pool_max=pool_max
            )
...
class MetricsCollector:
    ...
    def _archive_yesterday(self, yesterday_date: str, data: Dict):
        ...
        if yesterday_date:
            yesterday_stats = {
                ...
                "value_protected": sum(
                    t.get("amount", 0)
                    for t in data.get("transactions_today", [])
                    if t.get("is_fraud")
                ),
            }
    ...
    def get_kpis(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            if transactions_yesterday > 0:
                approval_rate_yesterday = (
                    (transactions_yesterday - frauds_yesterday) / transactions_yesterday
                ) * 100

            total_value = sum(
                t.get("amount", 0) for t in self._transactions_today if t.get("is_fraud")
            )
    ...
    def get_channel_stats(self) -> List[Dict]:
        ...
        with self._lock:
            ...
            for channel in channels:
                stats = self._channel_stats.get(
                    channel, {"frauds": 0, "value": 0.0, "transactions": 0}
                )
                result.append(
                    {
                        "name": channel,
    ...
    def get_alerts(self) -> List[Dict]:
        ...
        with self._lock:
            ...
            kpis = self.get_kpis()
            if (
                kpis["fraudes_detectadas"] > kpis["fraudes_ontem"] * 1.5
                and kpis["fraudes_detectadas"] > 5
            ):
                alerts.append(
                    {
                        "id": 1,
...
def login():
    ...
    if not username or not password:
        return (
            jsonify({"success": False, "error": {"message": "Username and password are required"}}),
            400,
        )
    ...
    token = pyjwt.encode(
        token_payload, config.security.jwt_secret, algorithm=config.security.jwt_algorithm
    )
...
def verify_token():
    ...
    try:
        ...
        return (
            jsonify({"success": False, "valid": False, "error": {"message": "Token expired"}}),
            401,
        )
    except pyjwt.InvalidTokenError as e:
        return (
            jsonify(
                {"success": False, "valid": False, "error": {"message": f"Invalid token: {str(e)}"}}
            ),
            401,
        )
...
def refresh_token():
    ...
    try:
        ...
        new_token = pyjwt.encode(
            new_payload, config.security.jwt_secret, algorithm=config.security.jwt_algorithm
        )

        return jsonify({"success": True, "data": {"token": new_token, "expires_in": 86400}})
    except pyjwt.ExpiredSignatureError:
        return (
            jsonify({"success": False, "error": {"message": "Token expired, please login again"}}),
            401,
        )
    except pyjwt.InvalidTokenError as e:
...
def predict_fraud():
    ...
    if include_explanation and explainability_engine.model is not None:
        try:
            X_features = (
                fraud_engine.last_features if hasattr(fraud_engine, "last_features") else None
            )
    ...
    for i, pred in enumerate(predictions):
        ...
        observability_metrics.observe(
            "transaction_amount", float(transactions_data[i].get("amount", 0))
        )
...
def explain_transaction():
    ...
    try:
        ...
        explanation = explainability_engine.explain_prediction(
            X_features, transaction_id=txn_id, fraud_probability=pred.risk_score
        )
        ...
        if include_compliance:
            response_data["compliance_report"] = explainability_engine.to_compliance_report(
                explanation
            )
    ...
    try:
        ...
        data = {
            "amount": np.concatenate(
                [np.random.exponential(500, n_legit), np.random.exponential(2000, n_frauds)]
            ),
            "hour": np.random.randint(0, 24, n_samples),
            "day_of_week": np.random.randint(0, 7, n_samples),
            "location_risk_score": np.concatenate(
                [np.random.beta(2, 8, n_legit), np.random.beta(6, 3, n_frauds)]
            ),
            "device_risk_score": np.concatenate(
                [np.random.beta(2, 8, n_legit), np.random.beta(5, 3, n_frauds)]
            ),
            "velocity_score": np.concatenate(
                [np.random.beta(2, 8, n_legit), np.random.beta(6, 2, n_frauds)]
            ),
            "is_new_device": np.concatenate(
                [np.random.binomial(1, 0.1, n_legit), np.random.binomial(1, 0.6, n_frauds)]
            ),
    ...
    return jsonify(
        {
            "success": True,
            "data": paginated,
            "stats": {"total": len(formatted_transactions), "page": page, "limit": limit},
        }
    )
...
def approve_transaction(transaction_id):
    ...
    if success:
        ...
        logger.info(
            f"Transaction approved: {transaction_id}",
            extra={"action": "TRANSACTION_APPROVED", "transaction_id": transaction_id},
        )
    return jsonify(
        {
            "success": success,
            "message": (
                f"Transação {transaction_id} aprovada com sucesso"
                if success
...
def reject_transaction(transaction_id):
    ...
    reason = (
        request.json.get("reason", "Rejeitado por analista")
        if request.json
        else "Rejeitado por analista"
    )
    ...
    if success:
        postgres_store.add_audit_log(
            ...
            None,
            f"Transaction rejected: {transaction_id} - {reason}",
            request.remote_addr,
        )
        logger.info(
    ...
    return jsonify(
        {
            "success": success,
            "message": (
                f"Transação {transaction_id} rejeitada" if success else "Transação não encontrada"
            ),
...
def send_to_review(transaction_id):
    ...
    logger.info(
        f"Transaction sent to review: {transaction_id}",
        extra={"action": "TRANSACTION_SENT_TO_REVIEW", "transaction_id": transaction_id},
    )
    return jsonify(
        {
            "success": True,
...
def flag_transaction(transaction_id):
    ...
    logger.info(
        f"Transaction flagged: {transaction_id}",
        extra={
...
def create_investigation():
    ...
    logger.info(
        f"Investigation created: {investigation['id']}",
        extra={
    ...
    return jsonify(
        {
            ...
            "false_positives": 0,
            "accuracy": (
                model_metrics.get("metrics", {}).get("accuracy", 0) * 100
                if model_metrics.get("metrics", {}).get("accuracy")
                else 0
            ),
            ...
            "auto_learning_confidence": (
                model_metrics.get("metrics", {}).get("f1_score", 0) * 100
                if model_metrics.get("metrics", {}).get("f1_score")
                else 0
            ),
...
def add_to_manual_review():
    ...
    if success:
        postgres_store.add_audit_log(
            "MANUAL_REVIEW_ADD",
            None,
            f"Added to manual review: {transaction_id}",
            request.remote_addr,
        )
    return jsonify(
        {
            "success": success,
            "message": "Added to review queue" if success else "Transaction not found",
        }
    )
...
def complete_manual_review():
    ...
    return jsonify(
        {"success": success, "message": "Review completed" if success else "Transaction not found"}
    )
...
def update_manual_review(item_id: int):
    ...
    postgres_store.add_audit_log(
        "MANUAL_REVIEW_UPDATE", None, f"Updated review item: {item_id}", request.remote_addr
    )
...
def delete_manual_review(item_id: int):
    ...
    postgres_store.add_audit_log(
        "MANUAL_REVIEW_DELETE", None, f"Deleted review item: {item_id}", request.remote_addr
    )
...
def add_hard_rule():
    ...
    if "updated_at" in result and result["updated_at"]:
        result["updated_at"] = (
            result["updated_at"].isoformat() + "Z"
            if hasattr(result["updated_at"], "isoformat")
            else str(result["updated_at"])
        )

    postgres_store.add_audit_log(
        "HARD_RULE_ADD", None, f"Added hard rule: {name}", request.remote_addr
    )
...
def update_hard_rule(rule_id: int):
    ...
    if success:
        postgres_store.add_audit_log(
            "HARD_RULE_UPDATE", None, f"Updated hard rule ID: {rule_id}", request.remote_addr
        )
...
def delete_hard_rule(rule_id: int):
    ...
    if success:
        postgres_store.add_audit_log(
            "HARD_RULE_DELETE", None, f"Deleted hard rule ID: {rule_id}", request.remote_addr
        )
...
def get_hard_rules_metadata():
    ...
    metadata = {
        "fields": [
            ...
            {
                "value": "velocity_1h",
                "label": "Transações na Última Hora",
                "type": "number",
                "category": "velocity",
            },
            {
                "value": "velocity_24h",
                "label": "Transações nas Últimas 24h",
                "type": "number",
                "category": "velocity",
            },
            {
                "value": "amount_24h",
                "label": "Valor Total 24h",
                "type": "number",
                "category": "velocity",
            },
            {
        ...
        "operators": [
            {"value": "==", "label": "Igual a", "types": ["string", "number", "select", "boolean"]},
            {
                "value": "!=",
                "label": "Diferente de",
                "types": ["string", "number", "select", "boolean"],
            },
        ...
        "actions": [
            {
                "value": "block",
                "label": "Bloquear Transação",
                "description": "Rejeita a transação imediatamente",
            },
            {
                "value": "review",
                "label": "Enviar para Revisão",
                "description": "Envia para fila de análise manual",
            },
            {
                "value": "alert",
                "label": "Gerar Alerta",
                "description": "Cria alerta mas permite transação",
            },
            {
                "value": "approve",
                "label": "Aprovar Automaticamente",
                "description": "Aprova sem análise adicional",
            },
            {
                "value": "step_up",
                "label": "Autenticação Adicional",
                "description": "Solicita verificação extra",
            },
            {
                "value": "score_adjust",
                "label": "Ajustar Score",
                "description": "Modifica score de risco",
            },
        ],
        "rule_types": [
            {
                "value": "blocking",
                "label": "Regra de Bloqueio",
                "description": "Bloqueia transações suspeitas",
            },
            {
                "value": "scoring",
                "label": "Regra de Pontuação",
                "description": "Ajusta score de risco",
            },
            {
                "value": "routing",
                "label": "Regra de Roteamento",
                "description": "Direciona para filas específicas",
            },
            {
                "value": "alerting",
                "label": "Regra de Alerta",
                "description": "Gera alertas sem bloquear",
            },
        ],
        "logic_operators": [
    ...
    for cond in conditions:
        ...
        if field == "hour":
            ...
            risk_analysis.append(
                "Velocidade alta de transações indica possível ataque automatizado"
            )
...
def get_vip_list():
    ...
    for vip in vips:
        if "created_at" in vip and vip["created_at"]:
            vip["added_at"] = (
                vip["created_at"].isoformat() + "Z"
                if hasattr(vip["created_at"], "isoformat")
                else str(vip["created_at"])
            )
...
def add_to_vip_list():
    ...
    postgres_store.add_audit_log(
        "VIP_LIST_ADD",
        None,
        f"Added to VIP list: {identifier_type}:{identifier[:4]}***",
        request.remote_addr,
    )
...
def remove_from_vip_list(item_id: int):
    ...
    if success:
        postgres_store.add_audit_log(
            "VIP_LIST_DELETE", None, f"Removed from VIP list ID: {item_id}", request.remote_addr
        )
...
def add_to_hot_list():
    ...
    postgres_store.add_audit_log(
        "HOT_LIST_ADD",
        None,
        f"Added to Hot List: {identifier_type}:{identifier[:4]}***",
        request.remote_addr,
    )
...
def remove_from_hot_list(item_id: int):
    ...
    if success:
        postgres_store.add_audit_log(
            "HOT_LIST_DELETE", None, f"Removed from Hot List ID: {item_id}", request.remote_addr
        )
    return jsonify({"success": success, "message": "Deleted" if success else "Item not found"})
...
def update_settings():
    ...
    postgres_store.add_audit_log(
        "SETTINGS_UPDATE",
        None,
        f"Updated settings: {list(request.json.keys())}",
        request.remote_addr,
    )
...
def reset_settings():
    ...
    postgres_store.add_audit_log(
        "SETTINGS_RESET", None, "Settings reset to defaults", request.remote_addr
    )

    return jsonify(
        {"success": True, "message": "Configurações resetadas com sucesso", "data": updated}
    )
        ...
        extended_alerts.append(
            {
                "id": (
                    alert_id
                    if isinstance(alert_id, str) and alert_id.startswith("ALT")
                    else f"ALT-{alert.get('id', 0):04d}"
                ),
                ...
                "timestamp": (
                    alert.get("created_at").isoformat() + "Z"
...
def get_audit_logs():
    ...
    logs = postgres_store.get_audit_logs(
        limit=limit, action_filter=action_filter, start_date=start_date, end_date=end_date
    )
...
def export_audit_logs():
    ...
    return jsonify({"success": True, "download_url": "/api/audit/download/export_2025.csv"})
...
def update_calibration():
    ...
    postgres_store.add_audit_log(
        "CALIBRATION_UPDATE",
        None,
        f"Updated calibration: {list(request.json.keys())}",
        request.remote_addr,
    )
...
def apply_calibration_changes():
    ...
    if db_persistence.is_available:
        try:
            conn = db_persistence._pool.getconn()
            try:
                with conn.cursor() as cur:
                    ...
                    """,
                        (
                            "CALIBRATION_APPLIED",
                            json.dumps({"changes": list(new_config.keys())}),
                            "system",
                        ),
                    )
                    conn.commit()
...
def get_calibration_history():
    ...
    if not db_persistence.is_available:
        return jsonify(
            {"success": True, "data": [], "total": 0, "note": "Histórico não disponível"}
        )
...
def get_investigations():
    ...
    for inv in investigations:
        if "created_at" in inv and inv["created_at"]:
            inv["created_at"] = (
                inv["created_at"].isoformat() + "Z"
                if hasattr(inv["created_at"], "isoformat")
                else str(inv["created_at"])
            )

    active_count = len(
        [i for i in investigations if i.get("status") in ["flagged", "investigating"]]
    )
    ...
    return jsonify(
        {
            ...
            "data": {
                ...
                "total": len(filtered),
                "query": query,
                "filters": {"category": category, "min_size": min_size, "max_size": max_size},
            },
        }
    )
...
def generate_report():
    ...
    postgres_store.add_audit_log(
        "REPORT_GENERATED", None, f"Generated {report_type} report", request.remote_addr
    )
...
def download_report(report_id: str):
    ...
    postgres_store.add_audit_log(
        "REPORT_DOWNLOAD", None, f"Report {report_id} downloaded", request.remote_addr
    )
    ...
    return jsonify(
        {
            ...
            "data": {
                ...
                "user_history": {"total_transactions": 150, "fraud_count": 0, "avg_amount": 500.0},
            },
        }
    )
...
def submit_feedback_v2():
    ...
    postgres_store.add_audit_log(
        "FEEDBACK_SUBMIT",
        None,
        f"Feedback submitted for {transaction_id}: {feedback_type}",
        request.remote_addr,
    )
...
def get_observability_metrics():
    ...
    """
    return jsonify({"success": True, "data": observability_metrics.get_all_metrics()})
...
def get_sla_compliance():
    ...
    - TPS mínimo
    """
    return jsonify({"success": True, "data": alert_manager.get_sla_compliance()})
        ...
        return jsonify(
            {
                ...
                "data": {
                    ...
                    "threshold": fraud_engine.threshold,
                    "feature_count": (
                        len(fraud_engine.feature_names) if fraud_engine.is_trained else 0
                    ),
                    "is_trained": fraud_engine.is_trained,
                },
            }
...
def detailed_health():
    ...
    """
    health = health_checker.check_all()
    return jsonify({"success": True, "data": health.to_dict()})
...
def get_queue_metrics():
    ...
    return jsonify({"success": True, "data": async_task_queue.get_metrics()})
...
def batch_process_transactions():
    ...
    batch_result = batch_processor.process_batch(
        transactions_data, process_single, batch_size=batch_size
    )
    ...
    return jsonify(
        {
            ...
            "data": {
                ...
                "throughput_tps": (
                    round(batch_result.total / (batch_result.processing_time_ms / 1000), 2)
...
def generate_bahnsen_features():
    ...
    bahnsen_engineer.add_transaction_to_history(
        user_id=user_id, amount=amount, timestamp=timestamp, channel=channel
    )
...
def analyze_social_engineering():
    ...
    result = nlp_detector.analyze_text(text=text, source=source)
...
def predict_enriched():
    ...
    if isinstance(base_prediction_obj, (list, tuple)):
        base_prediction = (
            base_prediction_obj[0].to_dict()
            if hasattr(base_prediction_obj[0], "to_dict")
            else base_prediction_obj[0]
        )
    else:
        base_prediction = (
            base_prediction_obj.to_dict()
            if hasattr(base_prediction_obj, "to_dict")
            else base_prediction_obj
        )
    ...
    return jsonify({"success": True, "data": enriched.to_dict()})
